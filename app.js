...
document.addEventListener("DOMContentLoaded", () => {
   ...
   const DOM = {
      ...
      drawerDetailImgContainer: document.getElementById(
         "drawer-detail-img-container",
      ),
      drawerDetailImg: document.getElementById("drawer-detail-img"),
      drawerDetailName: document.getElementById("drawer-detail-name"),
      drawerDetailStatusWrapper: document.getElementById(
         "drawer-detail-status-wrapper",
      ),
      drawerSpecificationsGrid: document.getElementById(
         "drawer-specifications-grid",
      ),
   ...
   function updateRouter() {
      if (state.activeSession) {
         ...
         DOM.navProfileAvatar.textContent = state.activeSession.name
            .charAt(0)
            .toUpperCase();
         DOM.profileBigAvatar.textContent = state.activeSession.name
            .charAt(0)
            .toUpperCase();
   ...
   function showCard(cardName) {
      ...
      if (cardName === "login") DOM.loginCard.classList.remove("hidden");
         ...
         DOM.recoverCard.classList.remove("hidden");
   }
   ...
   DOM.registerForm.addEventListener("submit", (e) => {
      ...
      if (!pass || pass.length < 6) {
         showInputError(
            ...
            "Contraseña mínimo de 6 caracteres",
         );
         valid = false;
      }
   ...
   DOM.recoverForm.addEventListener("submit", (e) => {
      ...
      if (!email) {
         showInputError(
            document.getElementById("recover-email"),
            "El correo es obligatorio",
         );
         return;
      } else if (!validateEmail(email)) {
      ...
      showToast(
         `Se han enviado las coordenadas de reinicio al correo: ${email}`,
         "success",
      );
   ...
   DOM.changePasswordForm.addEventListener("submit", (e) => {
      ...
      if (!currPass) {
         showInputError(
            document.getElementById("change-password-curr"),
      ...
      if (!newPass || newPass.length < 6) {
         showInputError(
            ...
            "Nueva contraseña mínimo de 6 caracteres",
         );
         valid = false;
      }
      ...
      if (newPass !== confirmNewPass) {
         showInputError(
            document.getElementById("change-password-confirm"),
            "Las contraseñas no coinciden",
         );
         valid = false;
      }
      ...
      const userIdx = users.findIndex(
         (u) =>
            u.email.toLowerCase() === state.activeSession.email.toLowerCase(),
      );
   ...
   async function loadInitialData() {
      ...
      if (cachedChars && cachedEps) {
         ...
         const localCharEdits = JSON.parse(
            localStorage.getItem("ram_custom_characters") || "[]",
         );
         const localEpEdits = JSON.parse(
            localStorage.getItem("ram_custom_episodes") || "[]",
         );

         state.characters = state.characters.map((cachedChar) => {
            const localEdit = localCharEdits.find(
               (e) => e.id === cachedChar.id,
            );
      ...
      localStorage.setItem(
         "ram_characters_cache",
         JSON.stringify(STATIC_CHARACTERS),
      );
      localStorage.setItem(
         "ram_episodes_cache",
         JSON.stringify(STATIC_EPISODES),
      );
      setFeaturedCharacter();
   ...
   async function syncWithAPI() {
      try {
         ...
         const charRes1 = await fetch(
            "https://rickandmortyapi.com/api/character",
         );
         const charData1 = await charRes1.json();

         const charRes2 = await fetch(
         ...
         console.error(
            "API Fetching failed, falling back to local copies: ",
            err,
         );
         // Fallback
   ...
   // Tab triggers
   DOM.tabCharactersBtn.addEventListener("click", () =>
   ...
   function switchTab(tab) {
      ...
      // Style tabs
      DOM.tabCharactersBtn.className =
         "tab-nav-btn" + (tab === "characters" ? " active-tab" : "");
      DOM.tabEpisodesBtn.className =
         "tab-nav-btn" + (tab === "episodes" ? " active-tab" : "");
      DOM.navProfileBtn.className =
         "nav-profile-trigger" + (tab === "profile" ? " active-profile" : "");
   ...
   DOM.characterSearch.addEventListener("input", () => {
      state.characterFilters.search = DOM.characterSearch.value
         .trim()
         .toLowerCase();
      renderCharactersList();
   });
   ...
   function renderCharactersList() {
      ...
      let items = state.characters.filter((char) => {
         const nameMatch = char.name
            .toLowerCase()
            .includes(state.characterFilters.search);
         const statusMatch =
            state.characterFilters.status === "" ||
            char.status.toLowerCase() === state.characterFilters.status;
         const genderMatch =
            state.characterFilters.gender === "" ||
            char.gender.toLowerCase() === state.characterFilters.gender;
   ...
   DOM.episodeSearch.addEventListener("input", () => {
      state.episodeFilters.search = DOM.episodeSearch.value
         .trim()
         .toLowerCase();
      renderEpisodesList();
   });
   ...
   tableHeaders.forEach((th) => {
      th.addEventListener("click", () => {
         ...
         if (state.episodeFilters.sortBy === col) {
            state.episodeFilters.sortDir =
               state.episodeFilters.sortDir === "asc" ? "desc" : "asc";
         } else {
   ...
   function openDrawer(item, type) {
      ...
      DOM.drawerTitle.textContent =
         type === "character" ? "Ficha de Personaje" : "Ficha de Episodio";
      ...
      `;
         ...
         if (epCounts > 0) {
            ...
            const listText = item.episode
               .map((link) => {
                  const id = link.split("/").pop();
                  const epObj = state.episodes.find((e) => e.id == id);
                  return epObj
                     ? `${epObj.episode} - ${epObj.name}`
                     : `Episodio #${id}`;
               })
               .join("<br>");
            DOM.drawerListContent.innerHTML = `<div class="space-y-1">${listText}</div>`;
         } else {
            DOM.drawerListContent.innerHTML =
               '<span class="text-sub-light dark:text-sub-dark italic">Ningún episodio registrado.</span>';
         }
      } else {
         // Episode mode
      ...
      `;
         ...
         if (castCount > 0) {
            const listText = item.characters
               .slice(0, 15)
               .map((link) => {
                  const id = link.split("/").pop();
            ...
            DOM.drawerListContent.innerHTML =
               '<span class="text-sub-light dark:text-sub-dark italic">Ningún personaje listado.</span>';
         }
      }
   ...
   function setupEditFormInputs() {
      ...
      if (!navigator.onLine) {
         ...
         badge.className =
            "p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-bold text-center font-mono animate-pulse mt-4";
   ...
   DOM.drawerEditForm.addEventListener("submit", (e) => {
      ...
      if (type === "character") {
         ...
         if (!species) {
            showInputError(
               document.getElementById("edit-species"),
         ...
         const charEdits = JSON.parse(
            localStorage.getItem("ram_custom_characters") || "[]",
         );
         const editIdx = charEdits.findIndex((e) => e.id === item.id);
         if (editIdx !== -1)
            charEdits[editIdx] = { id: item.id, ...updatedItem };
         else charEdits.push({ id: item.id, ...updatedItem });
         localStorage.setItem(
            "ram_custom_characters",
            JSON.stringify(charEdits),
         );
         ...
         if (
            state.featuredCharacter &&
            state.featuredCharacter.id === item.id
         ) {
         ...
         if (!name) {
            showInputError(
               document.getElementById("edit-ep-name"),
               "El nombre es obligatorio",
            );
            valid = false;
         }
         if (!air_date) {
            showInputError(
               document.getElementById("edit-ep-date"),
         ...
         if (!episode) {
            showInputError(
               document.getElementById("edit-ep-code"),
   ...
   async function initApp() {
      ...
      if (!navigator.onLine) {
         DOM.connBadge.className =
            "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold glass-container text-amber-500 border-amber-500/25";
